...
> Reconstructed from existing source code (diff/PR/feature area), not from a ticket/PRD. Keep Main Flow, Requirements, Business/Validation Rules, and Acceptance Criteria in business/black-box language — describe what a caller/user observes (API request/response, status codes, stage/state names), not how the code does it. Function names, internal variables, file:line refs, and framework-internal types belong only in section 8's Technical Notes / Implementation Notes.
...
- [Function/class/file:line implementing the behavior]
- [Internal variable/flag names and what they compute, if relevant to writing tests]
- [Test files/fixtures already covering or needing to cover this behavior]
